...
- 수강 취소 시 INCR
- 교수가 강의 정원 수정 시: (신규 정원 - 기존 정원)만큼 직접 증감 후 SET (CourseService.updateCourse)
- 강의 정원 변경 시 Debezium Consumer가 RDS COUNT 쿼리로 잔여 자리 재계산 후 SET (CDC 경로)
...
- 대기자 취소 시 INCR
- 대기자 만료/거절 시 INCR (expireAndPromoteNext)
- 대기자 수락 완료 시 INCR (수강 전환 후 슬롯 반환, EnrollmentService)
- 교수가 대기 정원 수정 시: (신규 대기 정원 - 기존 대기 정원)만큼 직접 증감 후 SET (CourseService.updateCourse)
- 강의 정원 변경 시 재계산 후 SET
- 강의 폐강 시 DEL

---

### 3. 대기자 순번 관리 (Sorted Set)

```
key:   waitlist:zset:course:{courseId}
type:  Sorted Set
score: 등록 시각 (ms)
member: waitlistId (String)
TTL:   없음
초기화: 대기 등록 시 ZADD
예시:  waitlist:zset:course:3 → {score: 1750000000000, member: "42"}
```

score 오름차순 정렬 = 먼저 등록한 순서 = 대기 순번. `ZRANK`로 현재 순번을 동적으로 계산하기 때문에 취소/만료/수락이 발생해도 별도 UPDATE 없이 순번이 자동으로 당겨진다. B안 도입 전에는 DB rank 컬럼을 `UPDATE waitlist SET rank = rank - 1` bulk UPDATE로 갱신했으나 동시 취소 시 MySQL 데드락이 발생했다. ZSET 전환으로 데드락 구조 자체를 제거했다.

**사용 시점**
- 대기 등록 시: DB INSERT 후 `ZADD` (score = 등록 시각 ms)
- 순번 조회 시: `ZRANK` + 1 (0-indexed이므로 +1)
- 대기 취소 시: DB commit 후 `ZREM`
- 대기 만료/거절 시: DB commit 후 `ZREM` (expireAndPromoteNext afterCommit)
- 대기 수락 시: DB commit 후 `ZREM`
- 다음 순번 조회 시: `ZRANGE 0 2` 후 만료 대상 제외한 첫 번째 (expireAndPromoteNext)
- 강의 폐강 시: `DEL` (CourseEventConsumer afterCommit)
- DB 롤백 시: `ZREM` + 슬롯 INCR 원복 (afterCompletion STATUS_ROLLED_BACK)
...
| `waitlist:zset:course:{id}` | 첫 대기 등록 시 ZADD | 등록(ZADD)/취소·만료·수락(ZREM) / 폐강 시 DEL | 없음 |
